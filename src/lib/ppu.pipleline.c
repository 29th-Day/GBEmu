...
u8 fetch_sprite_pixels(int bit, u8 color, u8 bg_color) {
...
bool pipeline_fifo_add() {
    ...
    for (int i=0; i<8; i++) {
        ...
        u8 color = lcd_get_context()->bg_colors[hi | lo];
...
void pipeline_load_window_tile() {
    ...
    if (ppu_get_context()->pfc.fetch_x + 7 >= window_x &&
        ppu_get_context()->pfc.fetch_x + 7 < (u8)(window_x + YRES + 14)) {
